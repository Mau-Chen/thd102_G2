...
const app = Vue.createApp({
  ...
  methods: {
    Initial(){
      ...
      axios.get(`/thd102/g2/php/Member/orderlist.php?MEMBER_ID=${MEMBER_ID}`)
      .then(response => {
        const data = response.data;

        const result_inside = data.map((item)=>({
          name: item.display_hotelname,
          image: this.selectPicture(item.display_hotelname),
          price: item.NOWPRICE,
